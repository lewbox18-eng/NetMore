...
function renderOrder() {
  ...
  elements.mockPayButton.textContent = order.payment.status === "paid"
    ? "Payment received"
...
async function loadHealth() {
  try {
    ...
    state.paymentProvider = health.paymentProvider || "mock";
...
async function submitCheckout(event) {
  ...
  try {
    ...
    state.currentOrder = created.order;
    state.cart = [];
    persistCart();
    persistCurrentOrder();
    renderCart();
    ...
    if (checkout.payment.provider === "stripe" && checkout.payment.checkoutUrl) {
      setMessage("Redirecting to secure card checkout...", "success");
...
async function completePayment() {
  ...
  if (state.paymentProvider === "stripe") {
    setMessage("Redirecting to secure card checkout...", "neutral");
...
async function hydrateOrderFromUrl() {
  ...
  try {
    const payload = await api(`/api/orders/${encodeURIComponent(orderId)}`);
    state.currentOrder = payload.order;
    persistCurrentOrder();
    renderOrder();

    if (checkoutState === "success") {
      setMessage(`Payment captured for ${payload.order.orderNumber}. The plugin can claim it now.`, "success");
    } else if (checkoutState === "cancel") {
      setMessage(`Checkout was canceled for ${payload.order.orderNumber}. You can try again any time.`, "neutral");
    }
  } catch (error) {
    setMessage(error.message, "danger");
  } finally {
